...
def test_postgres_conn(mock_postgres_conn_vars):
    # Assert the Postgres connection fixture is populated as expected
...
def test_dags_integrity(dagbag):
    # 1. Assert there are no DAG import errors
    assert dagbag.import_errors == {}, f"Import errors found: {dagbag.import_errors}"
    print("===========")
    print(dagbag.import_errors)

    # 2. Assert all expected DAG IDs are present
    ...
    # 3. Assert the total number of loaded DAGs matches expectation
    assert dagbag.size() == 3
    print("===========")
    print(dagbag.size())

    # 4. Assert each DAG has the expected number of tasks
